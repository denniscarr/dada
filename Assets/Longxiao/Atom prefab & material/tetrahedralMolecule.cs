...
public class tetrahedralMolecule : MonoBehaviour {
	...
	void Start () {
		...
		Vector3 key1 = centerAtom.transform.position + new Vector3(((centerAtom.transform.localScale.x) / 2)+0.4f, -0.2f, 0);
		Vector3 key2 = centerAtom.transform.position + new Vector3 (0, ((centerAtom.transform.localScale.y) / 2)+0.4f, 0);
		GameObject boundingKey1 = Instantiate (key, key1, Quaternion.Euler(0,0,251));
		...
		sideAtom1.transform.position = boundingKey1.transform.position + boundingKey1.transform.up*sideAtom1.transform.localScale.x/2f;
